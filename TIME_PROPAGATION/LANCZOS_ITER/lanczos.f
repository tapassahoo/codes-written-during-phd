      ...
      DIMENSION AKI(10)
      DIMENSION NN(1)
      DIMENSION URTR(N_R/2+1:N_R),URTI(N_R/2+1:N_R)
      DIMENSION CKF(10)
      DIMENSION VIM(N_R)
...
CCCCCC
      CALL DEFAK(DR,AKR)
      CALL INITIAL_WEIGHT(EKIN,AM,CKI)
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
  ...
  50  ISTEP=ISTEP+1
      T=T+DDT
      IF(ISTEP.LE.NVIM) THEN
      CALL VPOTIM(DDT,R,ISTEP,VIM)
      ENDIF
      DO I=1,N_R
        X1(I)=PSIRE(I)
        Y1(I)=PSIIM(I)
     ...
     1RMST,RINV,SN_G,V0,PLAN1,PLAN2)
      ...
      WRITE(30+ISTEP/200,'(1X,5F12.6)')T,R(I),ABS(PS(I))
      ENDDO
      ENDIF
CCCCCC
      DO I=1,N_R
      ...
      ENDDO
      IF(ISTEP.EQ.8000)THEN
      DO I=N_R/2+1,N_R
      URTR(I)=PSRE(I)
      URTI(I)=PSIM(I)
      WRITE(80,'(1X,3F12.6)')R(I),URTR(I),URTI(I)
      ENDDO
      CALL FINAL_WEIGHT(VOLEM,R,URTR,URTI,CKF)
      ENDIF
CCCCCC
...
!                                                          !
!***********************************************************
      SUBROUTINE VPOTIM(DDT,R,ISTEP,VIM)
      IMPLICIT REAL*8 (A-H,O-Z)
      PARAMETER (N_R=512)
      PARAMETER (NVIM=4000)
      DIMENSION R(N_R)
...
CCCCCC
   ...
   40 CONTINUE
      ...
      END
C----------------------------------------------------------------------
      SUBROUTINE INITIAL_WEIGHT(EKIN,AM,CKI)
      IMPLICIT REAL*8 (A-H,O-Z)
      DIMENSION AKI(10),CKI(10)
CCCC
      AKMIN=15.0D0
      AKMAX=24.0D0
      PI=4.0D0*DATAN(1.0D0)
...
C
      ...
      WRITE(6,*)
      DO I=1,10
      AKI(I)=AKMIN+(I-1)*1.0D0
      CKI(I)=SQRT(2.0D0/PI)*DELTAR
     1       *EXP(-2.0D0*DELTAR*DELTAR*(AKI(I)-AK_0)**2)
      WRITE(6,44)I,AKI(I),CKI(I)
      ENDDO
      WRITE(*,*)
  44  FORMAT(1X,I4,4X,3F16.6)
      ...
      END
CCCCCC
      SUBROUTINE FINAL_WEIGHT(VOLEM,R,URTR,URTI,CKF)
      IMPLICIT REAL*8 (A-H,O-Z)
      PARAMETER (N_R=512)
      DIMENSION AKF(10)
      DIMENSION CKF(10)
...
CC
      DO I=1,10
        AKF(I)=AKMIN+(I-1)*1.0D0
        SUM=0.0D0
        DO J=N_R/2+10,N_R
          CSF=DCOS(AKF(I)*R(J))
          SSF=DSIN(AKF(I)*R(J))
          SUM=SUM+(URTR(J)*CSF+URTI(J)*SSF)
     1           +(URTI(J)*CSF-URTR(J)*SSF)
        ENDDO
        CKF(I)=SUM*VOLEM/DSQRT(2.0D0*PI)
        WRITE(6,44)I,AKF(I),CKF(I)
      ENDDO
  44  FORMAT(1X,I4,4X,3F16.6)
      RETURN
      END

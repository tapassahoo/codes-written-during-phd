...
!***********************************************************
  50  ISTEP=ISTEP+1
      ...
      DO I=1,N_R
        ...
        Y1(I)=PSIIM(I)
      ENDDO
      CALL ENERGY(X1,Y1,AKR,V0,VAL1,VAL2,VAL,VOLEM)
        EER1=VAL1*VOLEM
        EER2=VAL2*VOLEM
        EER=VAL*VOLEM
        MMM=NLAN
      CALL LANCZ(X1,Y1,DDT,VOLEM,RAV,MMM,ISTEP,AKR,V0)
      RAV=RAV*VOLEM
...
CCCCCC
     ...
     1           +V0(I)*Y0(I)
      ...
      END
CCCCCC
      SUBROUTINE LANCZ(X1,Y1,DT,VOLEM,RAV,M,ISTEP,AKR,V0)
      IMPLICIT REAL*8 (A-H,O-Z)
      PARAMETER (N_R=512)
      PARAMETER (LAD=100)
      DIMENSION X1(N_R),Y1(N_R)
...
!                                                          *
!***********************************************************
      SUBROUTINE ENERGY(X0,Y0,AKR,V0,VAL1,VAL2,VAL,VOLEM)
      IMPLICIT REAL*8(A-H,O-Z)
      PARAMETER (N_R=512)
      DIMENSION AKR(N_R)
      DIMENSION V0(N_R)

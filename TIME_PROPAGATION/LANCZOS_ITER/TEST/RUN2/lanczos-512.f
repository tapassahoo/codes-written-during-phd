...
!***********************************************************
  50  ISTEP=ISTEP+1
      CALL VPOTIM1(R,VIM)
C     IF(ISTEP.LE.NVIM) THEN
C     CALL VPOTIM2(DDT,R,ISTEP,VIM)
C     ENDIF
      DO I=1,N_R
        X1(I)=PSIRE(I)
        Y1(I)=PSIIM(I)
     ...
     1RMST,RINV,SN_G,V0,PLAN1,PLAN2)
      ...
      IF(MOD(ISTEP,400).EQ.0)THEN
      DO I=1,N_R
        PS(I)=DCMPLX(PSRE(I),PSIM(I))
        PS1(I)=PSIRE(I)*PSIRE(I)+PSIIM(I)*PSIIM(I)
        WRITE(30+ISTEP/400,'(1X,5F12.6)')T,R(I),ABS(PS(I)),PS1(I)
      ENDDO
      ENDIF
CCCCCC
      DO I=1,N_R
      ...
      ENDDO
CCCCCC
      WRITE(90,'(1X,I8,2F12.6)')ISTEP,PSRE(N_R-11),PSIM(N_R-11)
      SUM=0.0D0
      DO I=1,N_R/2-11
        SUM=SUM+(PSRE(N_R/2+I)*PSRE(N_R/2+I)
     1     +PSIM(N_R/2+I)*PSIM(N_R/2+I))*VOLEM
      ENDDO
      WRITE(201,'(1X,I8,20X,E12.6)')ISTEP,SUM
      T=T+DDT
      CALL FLUX(PSRE,PSIM,VOLEM,XG,T,ISTEP,AM,VAL2,VAL3)
CCCCCC
...
CCCCCC
      IF(INA.EQ.1)THEN
      DO I=1,NSTEP
        READ(90,'(1X,I8,2F12.6)')ISTEP,URE(I),UIM(I)
      ENDDO
CCCCCC
...
CCCCCC
      ...
      ENDDO
      DO IE=1,NE
        WRITE(8,*)ET(IE)*EPSKJ,AKI(IE),URR(IE)
      ENDDO
CCCCCC
      SUM=0.0D0
      DK=AKI(2)-AKI(1)
...
C     WRITE(102,*)R(I),VIM(I)
      ...
      END
CCCCCC
      SUBROUTINE VPOTIM2(DDT,R,ISTEP,VIM)
      IMPLICIT REAL*8 (A-H,O-Z)
      PARAMETER (N_R=512)
      PARAMETER (NVIM=2000)
      DIMENSION R(N_R)
      ...
      TT=DDT*NVIM
      ALPHA0=0.3D0
      ALPHA=ALPHA0*(1.0D0-T/(3.0D0*TT))
      DO I=1,N_R
         VIM(I)=0.0D0
         IF(R(I).LT.19.0D0.AND.R(I).GT.-19.0D0) GO TO 10
         IF(R(I).GT.19.0D0)THEN
         ARG=(R(N_R)-R(I))/ALPHA
         VIM(I)=VMAX/DCOSH(ARG)/DCOSH(ARG)
         ENDIF
         IF(R(I).LT.-19.0D0)THEN
         ARG=(R(1)-R(I))/ALPHA
         VIM(I)=VMAX/DCOSH(ARG)/DCOSH(ARG)
         ENDIF
  10     CONTINUE
...
C
  ...
  44  FORMAT(1X,I4,4X,3F16.6)
      ...
      END
CCCCCC
      SUBROUTINE FLUX(PSIRE,PSIIM,VOLEM,XG,X,ISTEP,AM,VAL2,VAL3)
      IMPLICIT REAL*8(A-H,O-Z)
      PARAMETER (N_R=512)
      DIMENSION PSIRE(N_R),PSIIM(N_R)
      HBAR=0.06350781278D0
      HST=0.5D0*(X-XG)
C     PSIREDR=(PSIRE(N_R-11)-PSIRE(N_R-13))/2.0D0/VOLEM
C     PSIIMDR=(PSIIM(N_R-11)-PSIIM(N_R-13))/2.0D0/VOLEM
      PSIREDR=(-PSIRE(N_R-10)+8.0D0*PSIRE(N_R-11)
     1       -8.0D0*PSIRE(N_R-13)+PSIRE(N_R-14))/12.0D0/VOLEM
      PSIIMDR=(-PSIIM(N_R-10)+8.0D0*PSIIM(N_R-11)
     1       -8.0D0*PSIIM(N_R-13)+PSIIM(N_R-14))/12.0D0/VOLEM
      VAL1=PSIRE(N_R-12)*PSIIMDR-PSIIM(N_R-12)*PSIREDR
      VAL2=HST*(HBAR/AM)*(VAL1+VAL3)+VAL2
      VAL3=VAL1
      WRITE(104,'(1X,I8,3F12.6)')ISTEP,X,XG,VAL2
      XG=X
      RETURN
      END
CCCCCC

...
CCCCCC
      ...
      DO I=1,N_R
        ...
        SS=SS+PS(I)*VOLEM
        WRITE(30,'(1X,I8,4F12.6)')R(I),PS(I)
      ENDDO
      WRITE(6,'(1X,F12.6)')'SS=',SS
      DO I=1,N_R
        RH=R(I)
        CALL POT(RH,EN)
        V0(I)=EN
...
CCCCCC
      MMM=NCHEB
      CALL CHEBYSHEV(BSL,MMM,RAV,DDT,EGRIDP,EGRIDM,
     1AKR,V0,PSIRE,PSIIM)
      RAV=RAV*VOLEM
      IF (EPSL1.GT.0.0D0) THEN
        IF(RAV.LT.EPSL1) NCHEB=NCHEB-NCDEL
        IF(RAV.GT.EPSL2) NCHEB=NCHEB+NCDEL
        IF(NCHEB.GT.NCHMAX) NCHEB=NCHMAX
        IF(NCHEB.LT.NCHMIN) NCHEB=NCHMIN
      ENDIF
      WRITE(14,*)ISTEP,RAV,NCHEB
CCCCCC
      SS=0.0D0
      DO I=1,N_R
      ...
      DO I=1,N_R
        PS(I)=PSIRE(I)*PSIRE(I)+PSIIM(I)*PSIIM(I)
        WRITE(30+ISTEP/100,'(1X,5F12.6)')T,R(I),PS(I)
      ENDDO
      ENDIF
      WRITE(10,'(1X,I8,2F12.6)')ISTEP,PSIRE(N_R-MD),PSIIM(N_R-MD)
CCCCCC
      SUM=0.0D0
      DO I=N_R/2+1,N_R
...
CCCCCC
      ...
      END
CCCCCC
      SUBROUTINE CHEBYSHEV(BSL,NN,RAV,T,EGRIDP,EGRIDM,
     1AKR,V0,PSIRE,PSIIM)
      IMPLICIT REAL*8(A-H,O-Z)
      ...
      CALL RECURSION(AKR,V0,NN,EGRIDP,EGRIDM,PSIRE,PSIIM,PHICHE)
      DO I=1,N_R
...
CCCCCC
      ...
      END
CCCCCC
      SUBROUTINE RECURSION(AKR,V0,NN,EGRIDP,EGRIDM,PSIRE,PSIIM,PHICHE)
      IMPLICIT REAL*8(A-H,O-Z)
      COMPLEX*16 ZI,ARR,PHICHE
      PARAMETER (N_R=4096)
      DIMENSION AKR(N_R)
      DIMENSION V0(N_R)
      DIMENSION PSIRE(N_R),PSIIM(N_R)

...
C      DIMENSION PSIREIN1(NJRTP),PSIIMIN1(NJRTP)
      ...
      DIMENSION QRE1(0:NV,0:NJ,NCH,0:NJ+JTOT)
      DIMENSION QIM1(0:NV,0:NJ,NCH,0:NJ+JTOT)
      DIMENSION QRE2(0:NV,0:NJ,NCH,0:NJ+JTOT)
      DIMENSION QIM2(0:NV,0:NJ,NCH,0:NJ+JTOT)
      DIMENSION QRE3(0:NV,0:NJ,NCH,0:NJ+JTOT)
      DIMENSION QIM3(0:NV,0:NJ,NCH,0:NJ+JTOT)
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      DOUBLE PRECISION ATR(NRTP,IIIJMP),ATI(NRTP,IIIJMP)
      DIMENSION PTR1(0:NV,0:NJ,NCH,LPJMP)
      DIMENSION PTI1(0:NV,0:NJ,NCH,LPJMP)
      DIMENSION PTR2(0:NV,0:NJ,NCH,LPJMP)
      DIMENSION PTI2(0:NV,0:NJ,NCH,LPJMP)
      DIMENSION PTR3(0:NV,0:NJ,NCH,LPJMP)
      DIMENSION PTI3(0:NV,0:NJ,NCH,LPJMP)
      CALL MPI_INIT(IERR)
...
!
      ...
      ENDIF  !MPI
**********************************************************************
      CALL MPI_BCAST(PSIRE,NSJRTP,MPI_DOUBLE_PRECISION,MASTER        !  
     1              ,MPI_COMM_WORLD,IERR)                            !  
      CALL MPI_BCAST(PSIIM,NSJRTP,MPI_DOUBLE_PRECISION,MASTER        !  
     1              ,MPI_COMM_WORLD,IERR)                            !  
      CALL MPI_BCAST(IT,1,MPI_INTEGER,MASTER                         !  
     1              ,MPI_COMM_WORLD,IERR)                            !  
      CALL MPI_BCAST(MMM,1,MPI_INTEGER,MASTER                        !  
     1              ,MPI_COMM_WORLD,IERR)                            !  
      CALL MPI_BCAST(NLAN,1,MPI_INTEGER,MASTER                       !  
     1              ,MPI_COMM_WORLD,IERR)                            !  
      CALL MPI_BCAST(PRE,NSJRTP,MPI_DOUBLE_PRECISION,MASTER          !  
     1              ,MPI_COMM_WORLD,IERR)                            !  
      CALL MPI_BCAST(PIM,NSJRTP,MPI_DOUBLE_PRECISION,MASTER          !  
     1              ,MPI_COMM_WORLD,IERR)                            !  
**********************************************************************
      IF(MYID.EQ.MASTER)THEN
        NSENT=1
  ...
  229 CONTINUE
        DO NRCVR=1,NUMPROC-1
          ITAG=0
          CALL MPI_SEND(IIIN,1,MPI_INTEGER
     1                 ,NRCVR,ITAG,MPI_COMM_WORLD,IERR)
          IIIF=IIIN+IIIJMP-1
          IF(IIIF.GT.NSJT) IIIF=NSJT
          ITAG=1
          CALL MPI_SEND(IIIF,1,MPI_INTEGER
     1                 ,NRCVR,ITAG,MPI_COMM_WORLD,IERR)
          IIIN=IIIN+IIIJMP
          NSENT=NSENT+1
        ENDDO
        DO I=1,NUMPROC-1
          CALL MPI_RECV(ATR,IIIJMP*NRTP,MPI_DOUBLE_PRECISION
     1        ,MPI_ANY_SOURCE,MPI_ANY_TAG,MPI_COMM_WORLD,STATUS,IERR)
          ITAG1=STATUS(MPI_TAG)
          NRCVR1=STATUS(MPI_SOURCE)
          CALL MPI_RECV(ATI,IIIJMP*NRTP,MPI_DOUBLE_PRECISION
     1        ,NRCVR1,ITAG1,MPI_COMM_WORLD,STATUS,IERR)
          IIIN1=ITAG1
          IIIF1=IIIN1+IIIJMP-1
          IF(IIIF1.GT.NSJT) IIIF1=NSJT
          DO III=IIIN1,IIIF1
            DO KK=1,NRTP
              LL=KK+(III-1)*NRTP
              PSIRE(LL)=ATR(KK,III-IIIN1+1)
              PSIIM(LL)=ATI(KK,III-IIIN1+1)
            ENDDO
          ENDDO
        ENDDO
        IF(NSENT.LE.NTOT)GOTO 229
        ITAG=0
        DO NRCVR=1,NUMPROC-1
     ...
     1                 ,NRCVR,ITAG,MPI_COMM_WORLD,IERR)
      ...
      ELSE
  232   ITAG=0
        CALL MPI_RECV(IIIN,1,MPI_INTEGER
     1               ,MASTER,ITAG,MPI_COMM_WORLD,STATUS,IERR)
        IF(IIIN.LT.0)GOTO 231
        ITAG=1
        CALL MPI_RECV(IIIF,1,MPI_INTEGER
     1               ,MASTER,ITAG,MPI_COMM_WORLD,STATUS,IERR)
!
        CALL HAMILTONIAN(RMST,RINV,SNG,VOLEM,AK,EK,EKP,EKM,
     1  PRE,PIM,PSIRE,PSIIM,AKR,AKT,AKP,WIN1,WIN2,RHOD,SINN,COSN,
     2  SIND,COSD,SIN2D,V11,V12,V13,V21,V22,V23,V31,V32,V33,
     3  IIIN,IIIF,ATR,ATI,
     4  PLAN1,PLAN2,PLAN3,PLAN4,PLAN5)
!
        ITAG=IIIN
        CALL MPI_SEND(ATR,IIIJMP*NRTP,MPI_DOUBLE_PRECISION
     1               ,MASTER,ITAG,MPI_COMM_WORLD,IERR)
        CALL MPI_SEND(ATI,IIIJMP*NRTP,MPI_DOUBLE_PRECISION
     1               ,MASTER,ITAG,MPI_COMM_WORLD,IERR)
        GO TO 232
      ENDIF
  231 CONTINUE
...
C     1                 +PSIRE(JJ)**2+PSIIM(JJ)**2
     ...
     1                  +RHO(IR)*(PSIRE(JJ)**2+PSIIM(JJ)**2)*VOLEM
      ...
      WRITE(121,45)T,SS(1),SS(2),SS(3),SS1,ENER,RHAVT,RHAVT/SS1
   45 FORMAT(8F16.8)
      CALL FLUSH(121)
!
C      DO IS=1,NSTATE
...
**********************************************************************
  ...
  239 CONTINUE
        DO NRCVR=1,NUMPROC-1
          ITAG=2
          CALL MPI_SEND(LPN,1,MPI_INTEGER
     1                 ,NRCVR,ITAG,MPI_COMM_WORLD,IERR)
          LPF=LPN+LPJMP-1
          IF(LPF.GT.NJTJR) LPF=NJTJR
          ITAG=3
          CALL MPI_SEND(LPF,1,MPI_INTEGER
     1                 ,NRCVR,ITAG,MPI_COMM_WORLD,IERR)
          LPN=LPN+LPJMP
          NSENT=NSENT+1
        ENDDO
        DO I=1,NUMPROC-1
          CALL MPI_RECV(PTR1,LPJMP*(NJ+1)*(NV+1)*NCH
     1        ,MPI_DOUBLE_PRECISION
     1        ,MPI_ANY_SOURCE,MPI_ANY_TAG,MPI_COMM_WORLD,STATUS,IERR)
          ITAG1=STATUS(MPI_TAG)
          NRCVR1=STATUS(MPI_SOURCE)
          CALL MPI_RECV(PTI1,LPJMP*(NJ+1)*(NV+1)*NCH
     1        ,MPI_DOUBLE_PRECISION
     1        ,NRCVR1,ITAG1,MPI_COMM_WORLD,STATUS,IERR)
          CALL MPI_RECV(PTR2,LPJMP*(NJ+1)*(NV+1)*NCH
     1        ,MPI_DOUBLE_PRECISION
     1        ,NRCVR1,ITAG1,MPI_COMM_WORLD,STATUS,IERR)
          CALL MPI_RECV(PTI2,LPJMP*(NJ+1)*(NV+1)*NCH
     1        ,MPI_DOUBLE_PRECISION
     1        ,NRCVR1,ITAG1,MPI_COMM_WORLD,STATUS,IERR)
          CALL MPI_RECV(PTR3,LPJMP*(NJ+1)*(NV+1)*NCH
     1        ,MPI_DOUBLE_PRECISION
     1        ,NRCVR1,ITAG1,MPI_COMM_WORLD,STATUS,IERR)
          CALL MPI_RECV(PTI3,LPJMP*(NJ+1)*(NV+1)*NCH
     1        ,MPI_DOUBLE_PRECISION
     1        ,NRCVR1,ITAG1,MPI_COMM_WORLD,STATUS,IERR)
          LPN1=ITAG1-2
          LPF1=LPN1+LPJMP-1
          IF(LPF1.GT.NJTJR) LPF1=NJTJR
          DO LP=LPN1,LPF1
            DO IFCH=1,NCH
              DO KM=0,NV
                DO JR=0,NJ
                  QRE1(KM,JR,IFCH,LP)=PTR1(KM,JR,IFCH,LP-LPN1+1)
                  QIM1(KM,JR,IFCH,LP)=PTI1(KM,JR,IFCH,LP-LPN1+1)
                  QRE2(KM,JR,IFCH,LP)=PTR2(KM,JR,IFCH,LP-LPN1+1)
                  QIM2(KM,JR,IFCH,LP)=PTI2(KM,JR,IFCH,LP-LPN1+1)
                  QRE3(KM,JR,IFCH,LP)=PTR3(KM,JR,IFCH,LP-LPN1+1)
                  QIM3(KM,JR,IFCH,LP)=PTI3(KM,JR,IFCH,LP-LPN1+1)
                ENDDO
              ENDDO
            ENDDO
          ENDDO
...
!
      DO LP=0,NJ+JTOT
        DO KM=0,NV
          DO JR=0,NJ
            WRITE(24+LP,77)ISTEP,KM,JR,(QRE1(KM,JR,IFCH,LP),IFCH=1,NCH),
     1                                 (QRE2(KM,JR,IFCH,LP),IFCH=1,NCH),
     2                                 (QRE3(KM,JR,IFCH,LP),IFCH=1,NCH)
            WRITE(24+LP,77)ISTEP,KM,JR,(QIM1(KM,JR,IFCH,LP),IFCH=1,NCH),
     1                                 (QIM2(KM,JR,IFCH,LP),IFCH=1,NCH),
     2                                 (QIM3(KM,JR,IFCH,LP),IFCH=1,NCH)
          ENDDO
          WRITE(24+LP,*)
          WRITE(24+LP,*)
        ENDDO
...
!
     ...
     1                 ,NRCVR,ITAG,MPI_COMM_WORLD,IERR)
      ...
      ELSE
  242   ITAG=2
        CALL MPI_RECV(LPN,1,MPI_INTEGER
     1               ,MASTER,ITAG,MPI_COMM_WORLD,STATUS,IERR)
        IF(LPN.LT.0)GOTO 241
        ITAG=3
        CALL MPI_RECV(LPF,1,MPI_INTEGER
     1               ,MASTER,ITAG,MPI_COMM_WORLD,STATUS,IERR)
      ICHUNK=1
!$OMP PARALLEL
!$OMP$ DEFAULT(PRIVATE)
!$OMP$ SHARED(ICHUNK,JTT,DD,REQ,BETA,DM,EPS,RM12,RHO,THE,PHI,
!$OMP$ RSTAR,AAP,VALP,ZP,PSIRE,PSIIM,
!$OMP$ LPN,LPF,PTR1,PTI1,PTR2,PTI2,PTR3,PTI3)
!$OMP DO SCHEDULE(DYNAMIC,ICHUNK)
     ...
     2    PRE1,PIM1,PRE2,PIM2,PRE3,PIM3)
          DO KM=0,NV
            DO JR=0,NJ
              PTR1(KM,JR,IFCH,LP-LPN+1)=PRE1(KM,JR)
              PTI1(KM,JR,IFCH,LP-LPN+1)=PIM1(KM,JR)
              PTR2(KM,JR,IFCH,LP-LPN+1)=PRE2(KM,JR)
              PTI2(KM,JR,IFCH,LP-LPN+1)=PIM2(KM,JR)
              PTR3(KM,JR,IFCH,LP-LPN+1)=PRE3(KM,JR)
              PTI3(KM,JR,IFCH,LP-LPN+1)=PIM3(KM,JR)
            ENDDO
          ENDDO
        ENDDO
      ENDDO
...
!$OMP END PARALLEL
        ITAG=LPN+2
        CALL MPI_SEND(PTR1,LPJMP*(NJ+1)*(NV+1)*NCH,MPI_DOUBLE_PRECISION
     1               ,MASTER,ITAG,MPI_COMM_WORLD,IERR)
        CALL MPI_SEND(PTI1,LPJMP*(NJ+1)*(NV+1)*NCH,MPI_DOUBLE_PRECISION
     1               ,MASTER,ITAG,MPI_COMM_WORLD,IERR)
        CALL MPI_SEND(PTR2,LPJMP*(NJ+1)*(NV+1)*NCH,MPI_DOUBLE_PRECISION
     1               ,MASTER,ITAG,MPI_COMM_WORLD,IERR)
        CALL MPI_SEND(PTI2,LPJMP*(NJ+1)*(NV+1)*NCH,MPI_DOUBLE_PRECISION
     1               ,MASTER,ITAG,MPI_COMM_WORLD,IERR)
        CALL MPI_SEND(PTR3,LPJMP*(NJ+1)*(NV+1)*NCH,MPI_DOUBLE_PRECISION
     1               ,MASTER,ITAG,MPI_COMM_WORLD,IERR)
        CALL MPI_SEND(PTI3,LPJMP*(NJ+1)*(NV+1)*NCH,MPI_DOUBLE_PRECISION
     1               ,MASTER,ITAG,MPI_COMM_WORLD,IERR)
        GO TO 242
      ENDIF
  241 CONTINUE
...
*                                                                  *
********************************************************************
      SUBROUTINE HAMILTONIAN(RMST,RINV,SNG,VOLEM,AK,EK,EKP,EKM,
     1PRE,PIM,PSIRE,PSIIM,AKR,AKT,AKP,WIN1,WIN2,RHOD,SINN,COSN,
     2SIND,COSD,SIN2D,V11,V12,V13,V21,V22,V23,V31,V32,V33,
     3IIIN,IIIF,ATR,ATI,
     4PLAN1,PLAN2,PLAN3,PLAN4,PLAN5)
      IMPLICIT REAL*8(A-H,O-Z)
...
C      PARAMETER (JTR=JTOT)
      ...
      DIMENSION ARP(N_R,NT2,N_P)
      DIMENSION ARP1(N_R,NT2,N_P)
      DIMENSION ATR(NRTP,IIIJMP),ATI(NRTP,IIIJMP)
********************************************************************
*                                                                  *
*   This program is distributed with permission                    *
*                                                                  *
...
!$OMP PARALLEL 
!$OMP$ DEFAULT(PRIVATE)
!$OMP$ SHARED(ICHUNK,RMST,RINV,SNG,VOLEM,AK,EK,EKP,EKM,
!$OMP$ PRE,PIM,PSIRE,PSIIM,AKR,AKT,AKP,WIN1,WIN2,RHOD,SINN,COSN,
!$OMP$ SIND,COSD,SIN2D,V11,V12,V13,V21,V22,V23,V31,V32,V33,
!$OMP$ IIIN,IIIF,ATR,ATI,
!$OMP$ PLAN1,PLAN2,PLAN3,PLAN4,PLAN5)
!$OMP DO SCHEDULE(DYNAMIC,ICHUNK)
      DO III=IIIN,IIIF
        IIT=III
...
!
     ...
     1              +HPSIIM6+HPSIIM7+HPSIIM8+HPSIIM9+HPSIIM10
              ...
              ATR(KK,III-IIIN+1)=PSIRE(LL)
              ATI(KK,III-IIIN+1)=PSIIM(LL)
            ENDDO
          ENDDO
        ENDDO
      ENDDO

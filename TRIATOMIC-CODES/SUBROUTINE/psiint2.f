      ...
      COMPLEX*16 ZI,A1,XI,AA,FAC
      COMPLEX*16 SS,SP1,SP2,SS1,PSINIT 
...
c     IF(DN.LE.0.0D0) THEN
c     DN=0.0D0
c     ENDIF
      DNO=SQRT(DN) 
      SNT=DCOS(TH)/DNO
c     WRITE(144,55)RH,TH,PH,SNT
      CNT=-DSIN(TH)*DSIN(PH)/DNO
      AJOC=RH*DSIN(TH)/(2.0D0*DNO)
c     AJOC=RH*DSIN(2.0D0*TH)/(4.0D0*DNO*DNO)
      ARG=EXP(-BE*(R1-RE))
      PSIR=SQRT(BE)*AFI(KMORSE,S,ARG)
CCC   
      XI=AN*EXP(-ZI*AK_0*R2-A1*(R2-R2_0)**2)
CCC
c     FAC=SQRT(2.0D0*PI*AJOC*(2.0D0*LTOT+1))
c    1*PSIR*XI*(-1)**(JROT-LTOT)

      FAC=SQRT(2.0D0*PI*DABS(SNT)*AJOC*(2.0D0*LTOT+1)*0.5D0)
     1*PSIR*XI*(-1)**(JROT-LTOT)
c     FAC=SQRT(2.0D0*PI*(2.0D0*LTOT+1))*PSIR*XI*(-1)**(JROT-LTOT)
...
CCC
      ...
      ENDDO
      PSINIT(KK,L)=FAC*SS
c     SP1(L)=SP1(L)+SNT*AJOC*CONJG(PSINIT(KK,L))*PSINIT(KK,L)*DR*DT*DP
      ENDDO
      ENDDO
      ENDDO
      ENDDO
c     WRITE(6,*)'BEFORE NORMALIZATION'
c     WRITE(6,*)
c     DO L=1,2*JTOT+1
c     SP2=0.0D0
c     WRITE(6,44)L-JTOT-1,DBLE(SS1),DIMAG(SS1)
c     SP2=SP2+DBLE(SP1(L))
c     ENDDO
c     WRITE(6,*)SP2
c     WRITE(6,*)
c     WRITE(6,*)
...
c     IF(AM.GE.0.0D0) THEN
c     CLGD=CLGD
c     ELSE
c     CLGD=CLGD*(-1)**(-AJ+AJ_1+AJ_2)
c     ENDIF
      RETURN 
      END
      FUNCTION FACT(N) 
...
CCC
      ...
      SIXI=DCOS(T)/DENO
CCC
c     XI=DATAN(DCOS(T)/DSIN(T)/DCOS(P))
      XI=DASIN(DCOS(T)/SQRT(1.0D0-Y*Y))
c     IF(XI.LT.0) THEN
c     XI=XI+2.0D0*PI
c     ENDIF
      FAC=(-1)**KK*EXP(-ZI*KK*P+ZI*MM*XI)
     1*PLT(JTOT+1,MPJ+1)*PLP(JTOT+1,NPJ+1)
CCC
c     IF(MM.EQ.-2) THEN
c     FAC=(-1)**KK*EXP(-ZI*KK*P)*2.0d0*SIXI*CSXI
c    1*PLT(JTOT+1,MPJ+1)*PLP(JTOT+1,NPJ+1)*SQRT(2.0D0)
c     ENDIF
c     IF(MM.EQ.-1) THEN
c     FAC=(-1)**KK*EXP(-ZI*KK*P)*(CSXI-ZI*SIXI)
c    1*PLT(JTOT+1,MPJ+1)*PLP(JTOT+1,NPJ+1)
c     ENDIF
c     IF(MM.EQ.0) THEN
c     FAC=(-1)**KK*EXP(-ZI*KK*P)
c    1*PLT(JTOT+1,MPJ+1)*PLP(JTOT+1,NPJ+1)
c     ENDIF
c     IF(MM.EQ.1) THEN
c     FAC=(-1)**KK*EXP(-ZI*KK*P)*(CSXI+ZI*SIXI)
...
c     ENDIF
c     IF(MM.EQ.2) THEN
c     FAC=(-1)**KK*EXP(-ZI*KK*P)*(2.0D0*CSXI*CSXI-1.0D0)
c    1*PLT(JTOT+1,MPJ+1)*PLP(JTOT+1,NPJ+1)*SQRT(2.0D0)
c     ENDIF
c     WRITE(66,44)XI,DBLE(FAC),DIMAG(FAC)
